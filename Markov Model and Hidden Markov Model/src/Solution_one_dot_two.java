...
public class Solution_one_dot_two {
			...
			for(int j=0; j < length; j++){
				for(int k=0; k < length; k++){
					for(int l=0; l < length; l++){
						...
						currentProbability = transitionMatrix.get(indexOfWashingtonCity,i) *  transitionMatrix.get(i, j) * transitionMatrix.get(j, k) * transitionMatrix.get(k, l);
	...
	public static void main(String[] args) {
		...
		System.out.println("Starting from Washington, the Sequence of next four Cities that has largest probability based on MM is  "+ArrayUtils.toString(citiesHavingLargestProbability));
