...
public class solution_four {
	...
	public static Matrix logistics_gradient_descent(){
		int r = 3;
		int c = 1;
		...
		for(int i=0;i<gd_maxIter;i++){
			...
			for(int j=0; j< 10; j++){
				...
				Matrix l2normalizationVariable = Theta.times(2.0);
				Gradient = Gradient.plus(featureValuesInMatrix.times(bracketValue).plus(l2normalizationVariable));
			}
   ...
   public static double calculateExponentioalTerm( double exponentialPowerTerm ){		
	...
	public static void calculateGPA(Matrix Theta){
			...
			int[] predictedGPA = new int[maxExample];
	 		...
	 		System.out.println("Predicted GPA using logistics regression with l2 normalization: \n"+ArrayUtils.toString(predictedGPA));
			
		}
		
	
	public solution_four(){
	    theta=new double[3];
		...
		public static void main(String[] args) {
			Matrix Theta = logistics_gradient_descent();
			System.out.println("The estimated theta by logistics normalization gradient descent: \n");
